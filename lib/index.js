...
Zen.indexHtml = function indexHtml (pageType, forS3) {
  const key = 'ZEN_SCRIPTS'
  let deps = ['lib/latte.js']
  if (pageType == 'head') {
    deps.unshift('icons')
    deps.push('node_modules/svelte/store.umd.js', 'node_modules/fuzzysort/fuzzysort.js', 'svelte/mini.js', 'svelte/command.js', 'ui/dist.js')
  }
  ...
  let elements = deps.flatten().compact(true).map(d => `<script src='${d}'></script>`)
  ...
  elements.unshift('<div id="zen"></div>')


  return Zen.config.htmlTemplate.replace(key, elements.join('\n'))
}
