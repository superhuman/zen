...
export default class Util {
  ...
  iconCache = null
  static async UtserveIcons(req, res) {
    if (this.iconCache) return res.end(this.iconCache)
    let icons = {}
    ...
    this.iconCache = 'Zen.icons = ' + JSON.stringify(icons)
    res.end(this.iconCache)
  }

  static wsSend(ws, obj) {
  ...
  static readFileAsync(p, encoding) {
    if (encoding === undefined) encoding = 'utf8'
    Util.ensureDir(path.dirname(p))
