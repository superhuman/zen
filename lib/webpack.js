...
module.exports = class WebpackAdapter extends EventEmitter {
  constructor() {
    ...
    wcfg.plugins.push(
      new webpack.ProgressPlugin((pct, message, addInfo) => {
        if (pct > 0 && pct < 1)
          this.onStats({
            status: 'compiling',
    ...
    this.compiler.hooks.invalid.tap('Zen', () =>
      this.onStats({ status: 'compiling', errors: [] })
    )
    this.compiler.hooks.compile.tap('Zen', () =>
      this.onStats({ status: 'compiling', errors: [] })
    )
    this.compiler.hooks.failed.tap('Zen', (error) =>
      this.onStats({ status: 'failed', errors: [error] })
    )
  ...
  onStats(stats) {
    if (stats.compilation) {
      ...
      stats.entrypoints = stats.compilation.entrypoints
        .get('bundle')
        .chunks.map((chunk) => chunk.files[0])
