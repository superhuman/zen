...
module.exports = class Server {
  constructor() {
    ...
    if (Zen.webpack) {
      ...
      // when there is some change in the dev server we notify
  ...
  filterTests(msg) {
    ...
    // what happens if we are not done?
  ...
  onWebsocket(ws) {
    ...
    ws.on('message', (msg) => {
      ...
      this.sendStatus() // is thi an infinite loop since the send status sends a message?
    })
    ws.on('error', (err) => {
  ...
  sendStatus() {
    // who is the status being sent to?
    wsSend(
      this.head,
      Object.assign(
        Object.select(
