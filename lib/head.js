...
function openCloudWatch() {
  ...
  if (focusedResult) {
    window.open(
      `https://${aws.region}.console.aws.amazon.com/cloudwatch/home?region=${aws.region}#logEventViewer:group=/aws/lambda/${Zen.config.lambdaNames.workTests};stream=${focusedResult.logStream}`
    )
  }
}
...
async function runIfCodeChanged() {
  ...
  if (compile.status == 'compiling') {
    console.log('runIfCodeChanged() compiling')
    return
  }
  if (hotReloading) {
    console.log('runIfCodeChanged() hotReloading')
    return
  }

  // HMR can't recover from compile errors. Once the build is good again, we need to reload
  if (compile.status == 'error') {
    console.log('runIfCodeChanged() error')
    return (compileHasFailed = true)
  }

  if (compileHasFailed) {
    console.log('runIfCodeChanged() compile fail reload')
  ...
  if (compile.hash && !window.ZenWebpackClient) {
    console.log('runIfCodeChanged() window.ZenWebpackClient missing!')
    location.reload()
  }
  if (ZenWebpackClient.needsUpdate(store.get().compile.hash)) {
    console.log('runIfCodeChanged() needs update')
    location.reload()
  }

  if (!ZenWebpackClient.needsUpdate(compile.hash)) {
    console.log('runIfCodeChanged() NOT needs update')
    return
  }

  console.log('runIfCodeChanged() hot reloading!')
