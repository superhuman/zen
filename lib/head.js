...
async function runIfCodeChanged() {
  ...
  // TODO not sure about this part X.x
  if (compile.hash && !webpackClient) return location.reload()

  if (!webpackClient.needsUpdate(compile.hash)) return
  ...
  while (webpackClient.needsUpdate(store.get().compile.hash)) {
    await webpackClient.update()
  }
  hotReloading = false
