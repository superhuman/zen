...
async function runIfCodeChanged() {
  ...
  if (compileHasFailed)
    ...
    console.log('compile failed reload')
  ...
  if (compile.hash && !window.ZenWebpackClient) {
    console.log('loaded before compile')
    //location.reload()
  }
  ...
  // continue hot reloading until we'rppe up to date. NB that store.compile might change during
