...
async function runIfCodeChanged() {
  ...
  if (!window.ZenWebpackClient) {
    // Use sessionStorage to prevent infinite reload loops
    const reloadKey = 'zen_reload_' + compile.hash
    if (compile.hash && !sessionStorage.getItem(reloadKey)) {
  ...
  // Clear reload markers since ZenWebpackClient is now available
  Object.keys(sessionStorage).forEach(key => {
    if (key.startsWith('zen_reload_')) sessionStorage.removeItem(key)
  })
  
  if (!window.ZenWebpackClient.needsUpdate(compile.hash)) return
