...
const { readFile, readFileAsync, writeFile, invoke, getFilePathsRecursively } = require('./util')
...
module.exports = class S3Sync extends EventEmitter {
  ...
  async run(index) {
    ...
    const filepaths = await getFilePathsRecursively(Zen.config.webpack.buildFolder)
    // TODO: something is wrong with us getting the filepaths here.
    console.log('filepaths', filepaths)

    //listing all files using forEach
    for (let filepath of filepaths) {
      const data = await readFileAsync(`${Zen.config.webpack.buildFolder}/${filepath}`)
    ...
    let { needed } = await invoke(Zen, 'zen-sync', {
