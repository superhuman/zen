...
module.exports = class S3Sync extends EventEmitter {
  constructor() {
    super()
    this.s3 = new AWS.S3({
      params: { Bucket: Zen.config.aws.assetBucket },
      maxRetries: 10,
    })
  ...
  _readBuildFileFromDisk(path) {
    return util.readFileAsync(path.replace('webpack/', 'build/'))
  }
  ...
  async run(index) {
    ...
    await workInPool(Object.values(this.files), 10, async (f) => {
      ...
      if (!f.hash) {
        let body = f.body || (await this._readBuildFileFromDisk(f.path))
