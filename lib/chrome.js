...
module.exports = class ChromeWrapper {
  ...
  launchLocal(opts) {
    ...
    this.getBrowser = ChromeLauncher.launch({
      ...
      return Util.writeFile(path.join(Zen.config.tmpDir, 'chrome.pid'), chrome.pid)
    }).then(() => {
      return CDP({ port: opts.port })
          .catch((err) => { console.error('Non-fatal error ChromeWrapper.launchLocal', err) })
    })
  }
  ...
  async launchLambda() {
    ...
    this.getBrowser = await CDP({ host: 'localhost', port: 9222 })
  }
