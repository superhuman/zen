;(function () {
  ...
  async function resizeWindow({ height = 600, width = 800 }) {
    await globalThis.zenResizeWindow({ height, width })
  }

  Latte.setup({
    mode: globalThis.zenIsHeaded() ? 'debug' : 'headless',
    willHotReload: true,
    helpers: {
      resizeWindow,
    },
  ...
  Latte.onTest = async function onTest(test, err, log) {
    if (err && typeof err == 'string') err = new Error(err) // in case the user throws a string
